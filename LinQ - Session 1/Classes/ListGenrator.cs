...
namespace LinQ___Session_1.Classes
{
    internal static class ListGenrator
    {
         ...
         static ListGenrator()
        {
            ...
            CustomersList = XDocument.Load("Customers.xml").Root?.Elements("customer").Select(c => new Customer
            {
                 ...
                 Orders = c.Element("orders")?.Elements("order").Select(o=> new Order()
                 {
                     ...
                     Total = decimal.TryParse((string?)o.Element("total"), out decimal total) ? total : 0,
